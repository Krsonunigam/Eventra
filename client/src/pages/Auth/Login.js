  ...
  const onSubmit = async (data) => {
    ...
    if (result.success) {
      ...
      if (selectedPlan) {
        ...
        setUserWithoutFaceData({ ...result.user, selectedPlan });
      ...
      if (result.user?.role === 'admin') {
        ...
        // Check if user has completed face training (only for non-admin users)
        if (!result.user?.faceDataCollected) {
          setUserWithoutFaceData(result.user);
          setShowFaceCapture(true);
